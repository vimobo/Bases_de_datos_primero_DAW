...
select gnpold, gnp, name, (gnp - gnpold) as dif from country;

-- 5. Listar el nombre, la cantidad de habitantes, la superficie y una columna llamada
--    'Densidad' con la densidad poblacional de todos los países.
--    (4 columnas, 239 registros)

select name, population, surfacearea, (population / surfacearea) as densidad from country order by densidad desc;

-- 6. Listar toda la información de los países, ordenados por población ascendente.
--    (15 columnas, 239 registros)

select * from country order by population asc;

-- 7. Listar nombre de los lenguajes en orden alfabético.
--    (1 columna, 984 registros)
...
--    (2 columnas, 20 registros)

select name, population from city order by population limit 20;

-- 9. Listar código, nombre y año de independencia de todos los países,
--    ordenados por antigüedad descendente.
--    (3 columnas, 239 registros)

select code, name, indepyear from country order by indepyear;

-- 10. Listar nombre y continente de los cien países con mayor expectativa de vida.
--     En caso de empate, mostrar primero el de menor superficie.
--     (2 columnas, 100 registros)

select 
name,
 continent, 
 lifeexpectancy from country order by lifeExpectancy limit 100;

-- 11. Listar todos los datos de los países que no cuenten con habitantes.
--     (15 columnas, 7 registros)

select * from country where population = null;

-- 12. Listar todos los datos de los países cuya expectativa de vida supere los 75 años,
--     ordenados de forma ascendente.
--     (15 columnas, 62 registros)

select * from country where lifeExpectancy > 75;

-- 13. Listar todos los datos de los países cuya independencia haya ocurrido
--     desde la segunda mitad del siglo XIX y cuya forma de gobierno sea
--     una monarquía constitucional.
--     (15 columnas, 20 registros)

select * from country where indepyear > 1950 and governmentform = 'Constitutional Monarchy';

-- 14. Listar todos los datos de los diez países europeos de mayor PBN.
--     (15 columnas, 10 registros)

select * from country order by gnp desc limit 10;

-- 15. Listar todos los datos de los países cuyo nombre coincida con su nombre local.

select * from country where localname = name;

--     (15 columnas, 104 registros)

-- 16. Listar todos los datos de los países cuya independencia se haya dado
--     a partir de la segunda mitad del siglo XX.
--     (15 columnas, 110 registros)

select * from country where indepyear > 1950;

-- 17. Listar todos los datos de los países situados en Europa, Asia o Sudamérica.
--     (15 columnas, 111 registros)

select * from country where continent = 'Asia' or continent = 'Europe' or continent = 'South America';


-- 18. Listar todos los datos de todos los países, excepto los africanos.
--     (15 columnas, 181 registros)

select * from country where continent != 'Africa';


-- 19. Listar todos los datos de las ciudades argentinas fuera de la provincia
--     de Buenos Aires.
--     (5 columnas, 26 registros)

select * from city where countrycode = 'ARG' and district != 'Buenos Aires';

-- 20. Listar todos los datos de las ciudades de entre 125.000 y 130.000 habitantes.
--     (5 columnas, 138 registros)

select * from city where population > 125000 and population < 130000;

-- 21. Listar todos los datos de los países sin datos de independencia.
--     (15 columnas, 47 registros)



-- 22. Listar todos los datos de los países sin datos de PBN anterior ni
--     de expectativa de vida.
--     (15 columnas, 17 registros)

-- 23. Listar todos los datos de los países cuyo nombre comience y termine con 'A'.
--     (15 columnas, 13 registros)

select * from country where name like 'A%';

-- 24. Listar todos los datos de los países cuyo nombre sea compuesto.
--     (15 columnas, 66 registros)

select * from country where name like '% %';


-- 25. Listar todos los datos de las ciudades cuyo nombre o distrito contengan un '-'.
--     (5 columnas, 372 registros)
...
--     de sus capitales (alias 'Capital').
--     (2 columnas, 14 registros)
...
-- 27. Listar el código de país, nombres de las ciudades y su población,
--     donde la expectativa de vida sea mayor a 80.
--     (3 columnas, 253 registros)
...
-- 28. Listar las capitales de los países cuya forma de gobierno sea
--     una República Federal.
--     (2 columnas, 15 registros)

select ci.name, c.governmentform
from country c
join city ci
on c.code = ci.countrycode
where c.capital = ci.id and governmentform = 'Federal Republic';

-- 29. Listar los lenguajes oficiales junto al nombre de sus países,
--     donde la población esté entre 1 y 3 millones.
--     (2 columnas, 14 registros)
...
where c.population > 1000000 and c.population < 3000000 and isofficial = 'T';

-- 30. Listar códigos, nombres locales y región de los países donde se hable español.
--     (3 columnas, 28 registros)
...
-- 31. Listar nombres y capitales de los países cuya capital concentre
...
join city ci
on c.capital = ci.id
where ci.population > (ci.population / 2) ;

-- 32. Listar nombres y superficie de países africanos cuya capital coincida
--     con el nombre del distrito.
--     (2 columnas, 32 registros)

select c.name, c.surfaceArea, ci.district
from country c
join city ci
on c.capital = ci.id
where c.name = ci.district;

-- 33. Listar nombres, capitales y año de independencia (sin nulos)
--     de los 20 países más antiguos.
--     (3 columnas, 20 registros)
...
order by c.indepyear limit 20;

-- 34. Listar ciudades junto a sus idiomas oficiales donde no se hable
--     español, inglés, portugués, italiano, francés ni alemán.
--     (2 columnas, 2694 registros)

select c.name, l.language, l.isOfficial
from city c
join countrylanguage l
on c.countrycode = l.countrycode
where l.isofficial = 'T' and (l.language != 'Spanish' and l.language != 'German' and  l.language != 'English'   );


-- 35. Listar nombre, población y país de las diez ciudades europeas
--     de habla inglesa más pobladas.
--     (3 columnas, 10 registros)

select c.name, ci.population, ci.name
from country c
join city ci
on c.code = ci.countrycode 
join countrylanguage l
...
-- 36. Mostrar población total, máxima, mínima, promedio y cantidad de registros
--     según la tabla de países.
--     (5 columnas, 1 registro)

select sum(population) as total,
		min(population) as minimo,
        max(population) as maximo,
		count(population) as registro,
        name
from country
group by name;
		

-- 37. Mostrar población total, máxima, mínima y promedio por continente.
--     (5 columnas, 7 registros)

select  continent,
		sum(population) as total,
		min(population) as minimo,
        max(population) as maximo,
        avg(population) as meida,
		count(population) as registro
from country
group by continent;

-- 38. Agrupar países por continente mostrando la cantidad de países por cada uno.
--     (2 columnas, 7 registros)

select count(name) as npaies,
		continent
from country
group by continent;

-- 39. Agrupar ciudades por país mostrando la sumatoria total de habitantes.
--     (2 columnas, 232 registros)

select c.name, co.name, sum(c.population) as poblacion
from city c
join country co
on c.countrycode = co.code
group by c.countrycode, co.name, c.name;


-- 40. Agrupar lenguajes mostrando el porcentaje mínimo registrado por cada uno.
--     (2 columnas, 457 registros)

select language, countrycode, min(percentage) from countrylanguage group by language, countrycode;

-- 41. Mostrar formas de gobierno de países europeos junto al promedio
--     de población correspondiente.
--     (2 columnas, 10 registros)



-- 42. Mostrar las diez regiones con mayor expectativa de vida promedio.
--     (2 columnas, 10 registros)

-- 43. Mostrar los distritos con mayor cantidad de ciudades cuya población
--     supere el promedio.
--     (2 columnas, 24 registros)

-- 45. Mostrar lenguajes junto a la cantidad de países donde son oficiales.
--     (2 columnas, 102 registros)

-- 46. Mostrar años de independencia (sin nulos) junto a la cantidad de países
--     que se independizaron ese año, solo cuando haya más de uno.
--     (2 columnas, 39 registros)

-- 47. Listar países junto a la cantidad de idiomas hablados,
--     solo aquellos entre 3 y 5 idiomas.
--     (2 columnas, 80 registros)

select count(l.language) as cuenta, c.name 
from countrylanguage l
join country c
on c.code = l.countrycode
group by c.name
having cuenta >= 3 and cuenta <=  5;

-- 48. Mostrar distritos junto al país al que pertenecen y su población total,
--     donde no supere los 10.000 habitantes.
--     (3 columnas, 35 registros)

select c.district, co.name, sum(c.population) as aa
from city c
join country co on c.countrycode = co.code
group by c.district, co.name
having aa < 10000;

-- 49. Mostrar regiones junto a su densidad poblacional promedio,
--     cuando supere la mitad de la densidad máxima.
--     (2 columnas, 3 registros)

select co.region, avg(c.population) as media from country co
join city c on c.countrycode = co.code
group by co.region
having media > (SELECT AVG((population / surfacearea)/2) AS densidad_media
        FROM country
        GROUP BY region);

-- 50. Mostrar lenguajes oficiales junto a su porcentaje promedio de habla,
--     cuando no supere un dígito entero.c.co
--     (2 columnas, 7 registros)


select  distinct countrycode, language, percentage from countrylanguage
where isOfficial = 'T'
having percentage < 1;
